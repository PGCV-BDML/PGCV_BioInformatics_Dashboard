export default function ServiceReportGeneratorLoading() {
  return (
    <div className="space-y-8 mx-auto font-aileron max-w-[1240px]">
      ...
      <div className="bg-surface border border-slate-300/70 rounded-[24px] p-4 md:p-6 shadow-xl shadow-slate-400/20">
        <div className="animate-pulse flex items-center gap-2 mb-6">
          ...
          <div className="h-6 bg-slate-200/40 rounded w-40" />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {Array.from({ length: 3 }).map((_, index) => (
            <div
              key={index}
              className="h-56 rounded-[28px] border border-slate-200 bg-slate-50/60 animate-pulse"
            />
          ))}
        </div>
      </div>
