...
export default async function InternshipProgramsPage() {
  ...
  const [{ data: programs }, { data: users }] = await Promise.all([
    supabase.from("training_program").select("*").eq("type", "internship"),
    supabase.from("users").select("*"),
  ]);

  if (!programs) throw new Error("Failed to load internship programs");
  ...
  const mappedPrograms: ProgramCard[] = programs.map((p) => ({
    ...
    end_date: p.end_date ?? "",
    duration: p.duration ?? "",
    participant_count: p.interns?.length ?? 0,
  }));
    ...
    <div className="space-y-8 mx-auto font-aileron w-full max-w-[1240px]">
      ...
      />
        ...
        <div className="bg-[#fffdf8] border border-slate-300/70 rounded-[24px] p-4 md:p-6 shadow-xl shadow-slate-400/20 space-y-6">
          <ProgramSearchGrid programs={mappedPrograms} type="internship" />
        </div>
      </div>
    );
  }
