...
import { forumBreadcrumbs } from "@/lib/breadcrumbs";
...
function ForumPageContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadError("Couldn't load Forum. Please refresh the page.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const closeAsk = () => {
    setIsAdding(false);
    if (addParam) router.replace(routes.forum.list);
  };
  ...
  const handleCreate = async (form: FaqThreadFormData) => {
    ...
    try {
      ...
      closeAsk();
      router.push(routes.forum.byId(created.id));
    } catch (error) {
  ...
  return (
    <div className="space-y-8 mx-auto font-aileron max-w-[1240px]">
      <PageHeader
        breadcrumbTrail={forumBreadcrumbs}
        title="Forum"
        subtitle="Staff discussion for pipelines, installs, and lab troubleshooting."
        actions={
          canAsk ? (
            <button
              type="button"
        ...
        {loadError ? (
          ...
          <ul className="divide-y divide-slate-100">
            {displayed.map((thread) => (
              <li key={thread.id}>
                <Link
                  href={routes.forum.byId(thread.id)}
